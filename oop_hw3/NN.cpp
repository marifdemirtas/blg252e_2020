...
double* Layer::computeNextLayer(double** weight, double* bias, int next_layer_size){
    /*
    ...
    */
    ...
    for (int i = 0; i < next_layer_size; i++)
    {
        ...
        {
            next_layer_vals[i] += weight[i][j] * neurons[j].getA();
        }
...
Network::Network(int layer_count, int* neuron_counts, int* neuron_types){
    ...
    weights = new double**[layer_count - 1];
    for (int i = 0; i < layer_count - 1; i++){
        weights[i] = new double*[neuron_counts[i + 1]];
        for (int j = 0; j < neuron_counts[i + 1]; j++)
        {
            weights[i][j] = new double[neuron_counts[i]];
    ...
    biases = new double*[layer_count - 1];
    for (int i = 0; i < layer_count - 1; i++)
    {
        biases[i] = new double[neuron_counts[i+1]];
        for (int j = 0; j < neuron_counts[i+1]; j++)
        {
...
void Network::forwardPropagate(double* init_vals){
    layers[0].setValues(init_vals);
